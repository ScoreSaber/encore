...
import type { FilesystemWriteScope } from '@/lib/filesystem/write-audit';
...
export async function readJsonFile<T>(jsonPath: string, schema: z.ZodType<T>, options: ReadJsonOptions<T> = {}) {
   const normalizedPath = resolveFilesystemPath(jsonPath);
...
export async function readJsonFileOrDefault<T>(jsonPath: string, schema: z.ZodType<T>, options: { defaultValue: T }) {
   const read = await readJsonFile(jsonPath, schema, { defaultValue: options.defaultValue });
   if (Result.isOk(read)) return read.value;

   return options.defaultValue;
}

export async function writeJsonFileAtomic<T>(jsonPath: string, value: T, schema: z.ZodType<T>, options: WriteJsonOptions) {
   ...
   const normalizedPath = resolveFilesystemPath(managedPath.value.path);
